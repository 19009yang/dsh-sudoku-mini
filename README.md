...
DeepSeek Harness Web 的数独插件，通过悬浮按钮打开约 340 px 的小型游戏面板。提供简单、中等、困难、专家四档，共 80 道本地唯一解题目，支持笔记、撤销、提示、暂停、自动保存和深色主题。
...
首次执行会下载 CLI。打开终端输出的 URL，即可看到数独悬浮按钮；已运行的 DSH Web 需重新启动并刷新页面。
...
- 点击悬浮按钮展开或收起面板；拖动按钮或桌面面板标题可移动位置。
...
- 按 `Escape` 先关闭内部确认，再收起面板；按 `Tab` 可返回宿主界面。
